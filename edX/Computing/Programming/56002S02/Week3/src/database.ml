...
let engine db query =
	match query with
		...
		| Search contact -> search db contact;; 

let (_, db, _) = engine db (Insert(c1));;
let (_, db, _) = engine db (Insert(c2));;
let (_, db, _) = engine db (Insert(c3));;
let (_, db, _) = engine db (Insert(c4));;
let (_, db, _) = engine db (Insert(c5));;
let (_, db, _) = engine db (Insert(c6));;
let (_, db, _) = engine db (Insert(c7));;
let (_, db, _) = engine db (Insert(c8));;
let (_, db, _) = engine db (Insert(c9));;

let (_, db, _) = engine db (Delete(c9));;
